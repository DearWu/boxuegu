define(['aside', 'header', 'loading', 'nprogress', 'jquery', 'template'], function (undefined, undefined, undefined, nprogress, $, template) {
 ...
 +	 * */
    ...
    var urlSearchArr = urlSearch.spilt('&');
    var urlSearchObj = {},
        temp;
    for (var i = 0; i < urlSearchArr.length; i++) {
        temp = urlSearchArr[i].spilt('=');
        urlSearchObj[temp[0]] = temp[1];
    }

    //判断参数中有没有tc_id,有则认为时编译，不是添加
    ...
    //添加讲师，成功后跳转到讲师列表页
    function teacherAdd(){
        //吧模板渲染到页面中，因为时添加，所以不需要回显数据，那么传递一个空对象即可

        $('.teacher').html(template('teacher-add-edit-tpl',{}));
        //提交，因为表单时动态生成的，所以使用委托的方式监听事件
        $(document).on('click','#teacher-add-edit-form',function(){
            $.post('/v6/teacher/add',$(this).serialize(),function(){
    ...
    function teacherEdit(){
        //回显，根据url传入的tc_id属性火球对应讲师的详细信息，然后回显数据
        $.get('/v6/teacher/edit',{tc_id:urlSeachObj.tc_id},function(data){
            $('.teacher').html(template('teacher-add-tpl',data.result));
        });
        //提交，表单时动态生成的，所以使用委托的方式监听事件
        $(document).on('click',"#teacher-add-edit-form",function(){
